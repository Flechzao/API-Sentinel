...
public class AdminController {
    ...
    public ResponseEntity<?> listUsers(@CookieValue(value = "SESSION_USER", required = false) String sessionUser) {
        ...
        return ResponseEntity.ok(userStore.getAllUsers());
    }

    /** Export all user data — same vertical-priv-esc pattern as /api/admin/users:
     *  only checks authentication, not admin role. SPA dashboard's hidden admin
     *  panel links here. Returns full user records (incl. passwordHash, ssn). */
    @GetMapping("/api/admin/export")
    public ResponseEntity<?> exportAll(@CookieValue(value = "SESSION_USER", required = false) String sessionUser) {
        if (sessionUser == null) {
            return ResponseEntity.status(401).body(Map.of("error", "not authenticated"));
        }
        return ResponseEntity.ok(userStore.getAllUsers());
